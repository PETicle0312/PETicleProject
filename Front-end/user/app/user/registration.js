...
export default function RegisterScreen() {
  ...
  const fetchSchools = async (keyword) => {
    try {
      const response = await axios.get(
        `http://220.86.166.180:8080/api/school/search?keyword=${keyword}` /*개인포트변경*/
      );
  ...
  const verifyStudent = async () => {
    ...
    try {
      const response = await axios.post(
        "http://220.86.166.180:8080/api/school/verify" /*개인포트변경*/,
        { studentNumber }
      );
  ...
  const verifyPhoneNumber = async () => {
    ...
    try {
      const response = await axios.post(
        "http://220.86.166.180:8080/users/verify-phone" /*개인포트변경*/,
        { phoneNumber: phone }
      );
      Alert.alert("인증 성공", response.data);
    } catch (error) {
  ...
  const checkIdDuplicate = async () => {
    ...
    try {
      const response = await axios.post(
        "http://220.86.166.180:8080/users/check-id" /*개인포트변경*/,
        // 아이디 중복 확인 API URL
        { userId: userId }
      );
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await axios.post(
        "http://220.86.166.180:8080/users/register" /*개인포트변경*/,
        {
          userId,
          password,
